...
def parse_flightpath_bytes(fp_bytes: bytes) -> np.ndarray:
    ...
    #fallback: simple DPT-like free text
...
def fig_deviation_time(coords: pd.DataFrame, intended: np.ndarray) -> plt.Figure:
    ...
    ax.plot(coords['elapsed_s'], devs_smooth, linewidth=1.5, label='Deviation')
...
def generate_report_bundle(
    ...
    """
    this is the main entry point for Dash.
    ...
    add_fig("xy_view_path.png",      fig_path_2d(coords, "xy", dims, intended, use_elapsed=True))
    add_fig("xz_view_path.png",      fig_path_2d(coords, "xz", dims, intended, use_elapsed=True))
    add_fig("yz_view_path.png",      fig_path_2d(coords, "yz", dims, intended, use_elapsed=True))
    add_fig("position_vs_time.png", fig_position_vs_time(coords))

    sections.append({
        "id": "trajectory",
        "title": "Trajectories",
        "figs": ["3d_path.png", "xy_view_path.png", "xz_view_path.png", "yz_view_path.png", "position_vs_time.png"]
    })
    ...
    if intended is not None and intended.size:
        add_fig("pathdeviation_vs_time.png", fig_deviation_time(coords, intended))
        add_fig("deviation_distribution.png", fig_deviation_hist(coords, intended))
        sections.append({
            "id": "accuracy",
            "title": "Continuous Accuracy",
            "figs": ["pathdeviation_vs_time.png", "deviation_distribution.png"],
    ...
    if wp_results:
        ...
        lines.append(f"- Waypoints hit within {tol:.2f} m: {wp_success_pct_local}% ({wp_hits}/{wp_total})")
        lines.append("")
        lines.append("| WP # | Error (m) |   Time   | Hit? |")
        lines.append("|:----:|:---------:|:--------:|:----:|")
        for i, r in enumerate(wp_results, start=1):  
            idx = i
            err = r['error_m']
            t = r['time']
    ...
    if not shifts.empty:
        ...
        lines.append(f"- Average: {shifts.mean():.1f} ms")
