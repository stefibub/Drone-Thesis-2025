...
def choose_equal_counts(dims: Tuple[float,float,float], target_total_cubes: int) -> Tuple[int,int,int]:
    ...
    best = (nx0, ny0, nz0)
    best_cost = (float("inf"), float("inf"))  # <-- FIX: tuple, not float
    ...
    return best
# -----------------------------
# Voxel grid utilities (CHANGED: equal subdivisions)
# -----------------------------
class VoxelGrid:
    """
    ...
    [
            ...
            children=[
                html.Div(
                    [
                        ...
                        html.Div(
                            "Allows for target flights to specific parts of the room which is being analysed.",
        ...
        html.Div(
            [
                    ...
                    [
                            ...
                            [
                                html.Div(
                                    [
                                        html.Span("W "),
                                        dcc.Input(
                                            id="room-w", type="number", value=ROOM_DIMS[0],
                                ...
                                html.Div(
                                    [
                                        html.Span("L "),
                                        dcc.Input(
                                            id="room-l", type="number", value=ROOM_DIMS[1],
                                            min=0.5, step=0.1, debounce=True, style={"width": "100%"},
                                        ),
                                    ],
                                    style={"flex": "1", "minWidth": "80px", "marginRight": "6px"},
                                ...
                                html.Div(
                                    [
                                        html.Span("H "),
                                        dcc.Input(
                                            id="room-h", type="number", value=ROOM_DIMS[2],
                                            min=0.5, step=0.1, debounce=True, style={"width": "100%"},
                                        ),
                                    ],
                            ...
                            [
                                html.Div(
                                    [
                                        html.Span("Floor "),
                                        dcc.Input(
                                            id="floor-clear", type="number", value=0.0,
                                ...
                                html.Div(
                                    [
                                        html.Span("Ceiling "),
                                        dcc.Input(
                                            id="ceil-clear", type="number", value=0.0,
                                            min=0.0, step=0.05, debounce=True, style={"width": "100%"},
                                        ),
                                    ],
                        ...
                        html.Div(
                            [
                                ...
                                dcc.Input(
                                    id="cube-id-input", type="number", min=1, step=1,
                ...
                # RIGHT CONTROLS
                        html.Div(
                            [
                                    ...
                                    [
                                        html.Div(
                                            [
                                                ...
                                                dcc.Input(
                                                    id="orbit-radius", type="number", value=0.6,
                                        ...
                                        html.Div(
                                            [
                                                ...
                                                dcc.Input(
                                                    id="orbit-views", type="number", value=12,
                                        ...
                                        html.Div(
                                            [
                                                ...
                                                dcc.Input(
                                                    id="orbit-dwell", type="number", value=2.0,
                                ...
                                # Flight summary card (INSIDE right-controls)
                                html.Div(
                                    id="flight-summary",
                                    style={
                                    "marginTop": "8px",
                                    ...
                                    "background": "#f7fbff",
                                    "lineHeight": "1.6",
                                    "fontSize": "inherit",      # <- was "13px"
                                    "fontFamily": "inherit",    # <- ensure same font family
                                },
                                    children=[
                                        html.H3("Flight summary", style=SECTION_TITLE_STYLE),  # <-- same title style
                                        html.Div("Select a cell to see the mission summary."),
                                    ],
                                ),
                            # >>> Add the button RIGHT HERE (inside rightControls) <<<
                                html.Button(
                                    "Generate flight report",
                                    id="btn-report",
                                    n_clicks=0,
                                    disabled=True,
                                    style=REPORT_BTN_STYLE_DISABLED,  # start disabled + gray
                                ),
                            ],
                ...
                # DOWNLOADS ROW (spans both columns)
                html.Div(
                    [
                        html.Button(
                            "Download CSV", id="btn-csv", n_clicks=0,
                            style={"padding": "8px 12px", "borderRadius": "8px"},
                        ),
                        html.Button(
                            "Download TXT", id="btn-txt", n_clicks=0,
                            style={"padding": "8px 12px", "borderRadius": "8px", "marginLeft": "8px"},
                        ),
                        html.Button(
                            "Download DPT", id="btn-dpt", n_clicks=0,
                            style={"padding": "8px 12px", "borderRadius": "8px", "marginLeft": "8px"},
                        ),

                        # Hidden download endpoints
...
def update_flight_summary(selected_data, radius, n_views, dwell, floor_clear, ceil_clear):
    ...
    # --- Status line with colour-coded badges (no emojis) ---
    ...
    children = [
        ...
        status_line,  # keep your coloured badges row
...
@app.callback(
    ...
    Output("btn-report", "style"),
    Input("btn-csv", "n_clicks"),
    Input("btn-txt", "n_clicks"),
    Input("btn-dpt", "n_clicks"),
)
def enable_report(btn_csv, btn_txt, btn_dpt):
    clicked_any = (btn_csv or 0) > 0 or (btn_txt or 0) > 0 or (btn_dpt or 0) > 0
    if clicked_any:
        return False, REPORT_BTN_STYLE_ENABLED
    else:
        return True, REPORT_BTN_STYLE_DISABLED
