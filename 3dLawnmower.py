...
def generate_cube_scan(
    ...
    # Top/bottom planar scans only when no overrides (same as before)
    ...
    # Original full interior height used to compute base density
    ...
    # Scale to reduced band
    if full_height > 0:
        scaled_nheight = math.ceil(original_nheight_full * (height_range / full_height))
    else:
        scaled_nheight = 1
    nheight = max(min_vertical_slices, scaled_nheight)
    dh = height_range / nheight  # step
        ...
        for i in range(nspan + 1):
            ...
            if exclude_endpoints:
                if nheight <= 1:
                    ...
                    if i % 2 == 0:
                        z_indices = range(1, nheight)
                    else:
                        z_indices = range(nheight - 1, 0, -1)
            else:
                if i % 2 == 0:
...
def visualize_waypoints_3d(
    ...
    label_every: int = 1,  # label every Nth waypoint; increase to reduce clutter
    ...
    for idx, (x, y, z) in enumerate(zip(xs, ys, zs), start=1):
        if (idx - 1) % label_every != 0:
            continue
        ax.text(x, y, z, str(idx), **text_kwargs)
...
if __name__ == '__main__':
    ...
    epsilon = 0.5  # small gap below the ceiling
    ceiling_z = ceiling_z - epsilon

    waypoints = generate_cube_scan(
        cfg,
        dims,
        overlap,
        wall_offset,
        ...
        min_vertical_slices=4,      # force at least 4 vertical subdivisions
        exclude_endpoints=True      # skip exact floor/ceiling if desired
