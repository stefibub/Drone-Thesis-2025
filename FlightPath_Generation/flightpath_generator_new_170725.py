...
class DroneConfig:
    ...
    """
    battery_time: float          # max flight time in seconds
    max_distance: float          # max travel distance in meters
    max_flight_time: float       # mission duration cap in seconds
...
def validate_mission(drone: DroneConfig, waypoints: List[Waypoint]) -> Tuple[bool, dict]:
    """
    check if mission fits drone limits
    returns (feasible, metrics) where metrics include distance and time
    """
    total_dist = 0.0
    travel_time = 0.0
    # sum distances and travel times between waypoints
    ...
    # check against battery and flight time
    allowed = min(drone.battery_time, drone.max_flight_time)
    feasible = (total_dist <= drone.max_distance and total_time <= allowed)
    return feasible, {'distance': total_dist, 'time': total_time}


if __name__ == '__main__':
    # example usage with default settings
    cfg = DroneConfig(
        battery_time   = 1200.0,  # seconds
        max_distance   = 5000.0,  # meters
        max_flight_time= 1100.0,  # seconds cap
    ...
    dims = (6.0, 6.0, 4.0)         # width, length, height in meters
