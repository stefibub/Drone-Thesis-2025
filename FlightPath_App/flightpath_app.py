                                        ...
                                        children=[
                                            html.Label(
                                                [
                                                    ...
                                                    html.Span(
                                                        " ⓘ",
                                                        title="How many distinct altitudes (layers) to fly.",
                                                        style=TIP_ICON_STYLE,
                                                    ),
                                                ],
                                                htmlFor="layers",
                                        ...
                                        children=[
                                            html.Label(
                                                [
                                                    ...
                                                    html.Span(
                                                        " ⓘ",
                                                        title="Meters between successive heights (vertical step). Start at Flight height z, then add Δz for each layer.",
                                                        style=TIP_ICON_STYLE,
                                                    ),
                                                ],
...
def make_3d_fig(df: pd.DataFrame, L: float, W: float, H: float, step_idx: int) -> go.Figure:
    ...
    if not df.empty:
        show = df[df["step"] <= step_idx]
        fig.add_trace(go.Scatter3d(x=show["x"], y=show["y"], z=show["z"],
                                   mode="lines+markers", name="path"))
        cur = df[df["step"] == min(step_idx, len(df)-1)].iloc[0]
        fig.add_trace(go.Scatter3d(x=[cur.x], y=[cur.y], z=[cur.z],
    ...
    by = [0, 0, W, W, 0]
    for z in [0, H]:
        fig.add_trace(go.Scatter3d(x=bx, y=by, z=[z]*5, mode="lines", showlegend=False))
    for (x, y) in [(0,0),(L,0),(L,W),(0,W)]:
        fig.add_trace(go.Scatter3d(x=[x,x], y=[y,y], z=[0,H], mode="lines", showlegend=False))

    # Lock scene ranges to room bounds + keep aspect to real dimensions
    fig.update_layout(
        scene=dict(
...
def render_metrics_card(metrics: dict, df: pd.DataFrame, mode: str) -> html.Div:
    ...
    # NOTE: if you want number of waypoints (points), use len(df).
    # You were using segments, so I keep len(df)-1:
    n_wps   = max(0, len(df) - 1)
    dist    = metrics.get("distance", 0.0)
    d_take  = metrics.get("distance_takeoff", 0.0)
    d_cru   = metrics.get("distance_cruise", 0.0)
    d_land  = metrics.get("distance_landing", 0.0)
    eta_min = metrics.get("total_time", 0.0) / 60.0
    batt    = metrics.get("battery_usage_percent", 0.0)
    corners = metrics.get("num_corners", None)

    # 3D extras if present
    seams = None
    if "vertical_seams_count" in metrics:
        seams = dict(
            count=int(metrics.get("vertical_seams_count", 0)),
            dist=metrics.get("vertical_seams_distance", 0.0),
            time=metrics.get("vertical_seams_time", 0.0),
        )

    # Build the facts list we’ll lay out in two columns
    facts = [
        ("Waypoints",       f"{n_wps}"),
        ("Total distance",  f"{dist:.1f} m"),
        ("Flight time",     f"{eta_min:.1f} min"),
        ("Battery usage",   f"{batt:.0f}%"),
    ]
    if corners is not None:
        facts.append(("90° turns", str(int(corners))))
    if seams:
        facts.append(("Vertical seams", f"{seams['count']}  ({seams['dist']:.1f} m, {seams['time']:.1f} s)"))

    # Each cell shows "label : value" with the value bolded
    cell_style = {"display": "flex", "justifyContent": "space-between", "gap": "8px"}
    grid = html.Div(
        [
            html.Div(
                [html.Span(f"{label}:"), html.Span(value, style={"fontWeight": 600})],
                style=cell_style,
            )
    ...
    # Compact breakdown line spanning both columns
    ...
    return html.Div(
        [html.Strong("Mission summary"), grid, breakdown_line],
...
def generate(nc, L, W, H, mode, overlap, wall_off, clearance, alt, turns, layers, layer_h, z_cap):
    try:
        # Input validation
        for name, val in [("Length", L), ("Width", W), ("Height", H), ("Altitude", alt)]:
            if val is None or val <= 0:
                return no_update, no_update, no_update, no_update, no_update, no_update, f"Invalid {name}"
        if not (0.0 <= overlap < 1.0):
            return no_update, no_update, no_update, no_update, no_update, no_update, "Overlap must be in [0,1)."

        dims = (float(L), float(W), float(H))
        drone = DEFAULT_DRONE

        if mode == "2D":
            base = generate_spiral_scan(drone, dims, float(alt), float(overlap), float(wall_off or 0), float(clearance or 0), int(turns or 3))
            ...
            base = generate_spiral_scan(drone, dims, float(alt), float(overlap), float(wall_off or 0), float(clearance or 0), int(turns or 3))
            n_layers = int(layers or 2)
            dz = float(layer_h or 0.4)
            # compute heights sequence, capped if provided
            heights = [max(drone.min_altitude, float(alt)) + i * dz for i in range(n_layers)]
            if z_cap is not None:
                heights = [z for z in heights if z <= float(z_cap)]
            if not heights:
                return no_update, no_update, no_update, no_update, no_update, no_update, "No valid layers under z cap."
