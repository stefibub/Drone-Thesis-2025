...
IDS = dict(
    ...
    btn_download_zip="btn-download-zip",
    dl_report_zip="dl-report-zip",    
)
...
def report_page_layout():
    ...
    return html.Div(
        ...
        children=[
            ...
            # download zip button (disabled until a report exists)
            html.Div(
                style={"display": "flex", "justifyContent": "center", "margin": "0 0 20px"},
                children=[
                    html.Button(
                        "Download report (.zip)",
                        id=IDS["btn_download_zip"],
                        n_clicks=0,
                        disabled=True,
                        style={
                            "padding": "10px 16px",
                        ...
                        title="Run the report first to enable download",
                    ),
                    dcc.Download(id=IDS["dl_report_zip"]),
                ],
            ),
...
@app.callback(
    Output(IDS["btn_download_zip"], "disabled"),
    Output(IDS["btn_download_zip"], "style"),
    Input(IDS["store_report_bundle"], "data"),
)
def enable_zip_download(report_bundle_json):
    base_style = {
        "padding": "10px 16px",
        "background": "#2563eb",
        "color": "white",
...
@app.callback(
    Output(IDS["dl_report_zip"], "data"),
    Input(IDS["btn_download_zip"], "n_clicks"),
    State(IDS["store_report_bundle"], "data"),
    prevent_initial_call=True,
)
def download_report_zip(n_clicks, report_bundle_json):
    if not n_clicks or not report_bundle_json:
        return no_update

    # the store holds JSON string for safety; decode it
    try:
        bundle = json.loads(report_bundle_json) if isinstance(report_bundle_json, str) else report_bundle_json
        zip_b64 = bundle.get("zip_b64")
        if not zip_b64:
            return no_update
        # dcc.Download accepts base64 payloads with base64=True
        return dict(
            content=zip_b64,
