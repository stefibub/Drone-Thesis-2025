...
################################## looking to see if it can be removed  ##############################
...
def generate_helical_trajectory(
    ...
    """
    Generates a 3D helical trajectory centered at the origin.
    radius: horizontal radius of the helix in meters
    vertical_pitch: vertical distance gained per full revolution
    revolutions: total number of revolutions (can be fractional)
    points_per_revolution: waypoints per full turn
    start_z: starting altitude
    gimbal_pitch: camera pitch angle in degrees
    speed: nominal speed
...
if __name__ == '__main__':
    ...
    cfg = DroneConfig(
        ...
        speed          = 0.5,            # m/s (used in validation)
        min_altitude   = 1.0,            # meters
        turning_radius = 1.0,            # meters
        hover_buffer   = 2,              # seconds
        battery_warning_threshold= 0.85  # fraction (85%)
    )

    # generate only a helical trajectory
    helix_wps = generate_helical_trajectory(
        radius=5.0,                # meters from center
        vertical_pitch=1.0,        # meters gained per revolution
        revolutions=4.0,           # total turns
        points_per_revolution=100, # resolution
        start_z=1.0,               # starting altitude
        gimbal_pitch=-45.0,        # camera angle
        speed=0.5,                 # used in waypoint objects
        hold_time=0.5              # seconds per waypoint hover
    )

    # validate mission feasibility
    feasible, metrics = validate_mission(cfg, helix_wps)

    print(f"Generated helix with {len(helix_wps)} waypoints.")
    ...
    # export and visualize helix: show both waypoints and the connecting path
    export_to_marvelmind(helix_wps, 'helix_waypoints.csv')
